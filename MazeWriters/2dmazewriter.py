...
# this text controls what the file name is. Default is 'MazeTrial2D.txt'
file = 'MazeTrial2D.txt'

# this text controls what you see. Default is 'C', '#', 'E'.
cell = 'C'
wall = '#'
error = 'E'
    ...
    with open(file, 'a') as draw:
        ...
        for y in range(0, len(maze)):  # for as many lines as there are in the maze
            for x in range(0, len(maze[0])):  # for as many characters as there are in the line
                if maze[y][x] == cell:  # if the character is a cell, make it a cell.
                    draw.write(cell + " ")
                elif maze[y][x] == wall:  # if the character is a wall, make it a wall.
                    draw.write(wall + " ")
                elif maze[y][x] == 'U':  # foreshadowing for the 3d maze code (still here)
                    draw.write("U ")
